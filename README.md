...
**Prisma P3009 (failed migration `20260412104500_subscription_option_duration_price`):** a previous `migrate deploy` can leave that migration marked **failed**, so new deploys stop. In Neon **SQL Editor**, run `docs/repair-p3009-subscription-option-migration.sql` (idempotent repair). Then on your machine with `DATABASE_URL` / `DIRECT_URL` pointing at that database, run `npm run db:resolve:subscription-migration-applied` (or `npx prisma migrate resolve --applied 20260412104500_subscription_option_duration_price`). If the schema was **already fully updated** before you found this doc, you can skip the SQL and only run the `resolve --applied` command. See [Prisma: production troubleshooting](https://www.prisma.io/docs/orm/prisma-migrate/workflows/troubleshooting-development).
