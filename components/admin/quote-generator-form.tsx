...
export function QuoteGeneratorForm({
  ...
  async function handleDownload(e: React.FormEvent) {
    ...
    const billToLines =
      selectedCustomer?.billToLines ??
      (clientName.trim() ? [clientName.trim()] : null);
  ...
  return (
    <form onSubmit={handleDownload} className="flex max-w-3xl flex-col gap-5">
      <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4 text-sm text-emerald-950 dark:border-emerald-900/40 dark:bg-emerald-950/25 dark:text-emerald-100">
        ...
        <p className="mt-1 text-emerald-900/90 dark:text-emerald-200/90">
          Fill in the client and line items, then download a branded PDF quote. For full sales proposals with visuals
          and terms, use{" "}
      ...
      <fieldset className="min-w-0 border-0 p-0">
        ...
        <p className="mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">
          Default rows match typical fleet tracking quotes — adjust quantities and prices before downloading.
        </p>
