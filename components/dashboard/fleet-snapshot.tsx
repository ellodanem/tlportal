...
} from "@/lib/admin/fleet-segments";

export type { FleetSegmentKey };
export { FLEET_SEGMENT_ORDER, FLEET_SEGMENT_META } from "@/lib/admin/fleet-segments";
...
export function FleetSnapshot({
  ...
  const gradientStops = rows.map((row) => {
    const color = FLEET_SEGMENT_META[row.key].color;
    ...
    <div className="mt-4 flex flex-col items-stretch gap-6 sm:flex-row sm:items-center">
      <div className="flex justify-center sm:justify-start">
        <Link
          ...
          aria-label="Open devices — fleet by segment"
        >
          <div
      ...
      <ul className="min-w-0 flex-1 space-y-2">
        {rows.map((row) => {
          const meta = FLEET_SEGMENT_META[row.key];
            ...
            <li key={row.key} className="flex items-center justify-between gap-3 text-sm">
              <span className="flex min-w-0 items-center gap-2">
                <span
                  ...
                  aria-hidden
                />
                <span className="min-w-0 truncate text-zinc-700 dark:text-zinc-300" title={meta.description}>
                  {meta.label}
                </span>
              </span>
