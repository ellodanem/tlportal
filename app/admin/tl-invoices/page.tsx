...
import { activeInvoiceWhere, archivedInvoiceWhere } from "@/lib/admin/active-invoice-filter";
...
export default async function AdminTlInvoicesPage({ searchParams }: Props) {
  ...
  const invoices = await prisma.invoice.findMany({
    where: showArchived ? archivedInvoiceWhere : activeInvoiceWhere,
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-zinc-600 dark:text-zinc-400">
            {showArchived
              ? "Hidden from the active list, AR aging, and billing reminders. Open an invoice to restore it."
              : "Unified AR — one-off, recurring, imported history, and Stripe subscription mirrors. Cash/cheque payers get native invoices; card subscriptions sync from Stripe automatically."}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          {showArchived ? (
            <Link
              ...
              className="inline-flex justify-center rounded-lg border border-zinc-300 bg-white px-4 py-2.5 text-sm font-semibold hover:bg-zinc-50 dark:border-zinc-600 dark:bg-zinc-900"
            >
              Active invoices
            </Link>
          ) : (
            <Link
              href="/admin/tl-invoices?view=archived"
              className="inline-flex justify-center rounded-lg border border-zinc-300 bg-white px-4 py-2.5 text-sm font-semibold hover:bg-zinc-50 dark:border-zinc-600 dark:bg-zinc-900"
            >
              View archived
            </Link>
          ...
          <table className="min-w-full divide-y divide-zinc-200 text-sm dark:divide-zinc-800">
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {invoices.map((inv) => (
                <tr
                  key={inv.id}
                  className={`text-zinc-800 dark:text-zinc-200 ${showArchived ? "opacity-80" : ""}`}
                >
