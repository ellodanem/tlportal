...
const SETTINGS_TABS = ["branding", "email", "alerts", "timezone"] as const;
...
export default async function AdminSettingsPage({ searchParams }: Props) {
  ...
  const activeTab = parseQueryTab(tabParam, SETTINGS_TABS, "branding");
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <SectionTabs
        ...
        active={activeTab}
        hrefFor={(tabId) => `/admin/settings?tab=${tabId}`}
      ...
      {activeTab === "alerts" ? <BillingAlertPhonesForm initialPhones={billingAlertPhones} /> : null}
